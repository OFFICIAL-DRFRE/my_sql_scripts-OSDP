Declare @popcode varchar(10)= ( select min(pop) from pop where active is null)
...
update pop set 
	ACCOUNT_TYPE=Null,
	owner_name= case 
					when owner_name is null then name 
					else Owner_name 
					end,
	Active='1', 
	AREATYPE='01',
	...
	USER_ENTRY= (select top 1 USER_ID from Users where status=1 and DESIGNATION=101),
	POPTYPE= case 
