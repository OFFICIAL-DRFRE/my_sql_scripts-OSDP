...
Declare @date date='20230120'
;with stock as (
select stk.distributor,stk.sku,cz= Isnull(floor(stk.pcs/NullIF(stk.SELL_FACTOR1,0)),0),
 ...
 (FLOOR((stk.pcs-(floor(stk.pcs/NullIF(stk.SELL_FACTOR1,0))*stk.SELL_FACTOR1))/(Nullif(stk.SELL_FACTOR2,0)))*stk.SELL_FACTOR2))) ,0)
  from(select distinct sb.distributor,b.SKU  'sku',--warehouse  '03',
...
(isnull(open_3,0)+isnull(IN_3,0)-isnull(sale_3,0)-isnull(OUT_3,0))*cs.SELL_FACTOR3)   'pcs',cs.SELL_FACTOR1,cs.SELL_FACTOR2,cs.SELL_FACTOR3
...
) select d.DISTRIBUTOR,d.NAME KD_Name, @date "Doc Date", st.sku,s.ldesc 'SKU desc',sum(cz) CS,sum(DZ) DZ,sum(PC) PC, 
sum(convert(int,(CZ*sell_factor1)+(DZ*sell_factor2)+PC)) 'Total in PC' from stock st
