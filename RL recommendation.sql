...
set @sqlscript = 
'select * from (
	select distinct VEHICLE, ph.LDESC PJP,m.LDESC SKU_desc, s.ldesc section_desc,
	case 
	...
	where moc_no = '+right('0'+convert( varchar, month(@sdate)), 2) +' and moc_year = '+convert(varchar, YEAR(@sdate))+'
		and pd.' + @col + ' = 1
	group by  VEHICLE, ph.LDESC,m.MASTER_SKU, m.LDESC, s.section, s.ldesc, m.SELL_FACTOR1
	) st
where iq_balance > 0
';
