...
year(cm.doc_date) 'Year', Month(cm.DOC_DATE)'Month',pj.pjp as  'pjp',
 ds.NAME AS NAME,S.LDESC 'SKU Name' ,c.sku,sum(c.amount) 'GSV',
 sum( ((C.QTY1*s.sell_factor1)+(QTY2*s.sell_factor2)+(QTY3*s.sell_factor3))) 'Number of pcs'
...
group by cm.distributor,d.NAME,CM.POP_NAME,SE.LDESC, cm.town+cm.locality+cm.slocality+cm.POP,  
year(cm.DOC_DATE), Month(cm.DOC_DATE),pj.PJP, ds.name,s.LDESC,c.sku 
order by year,Month