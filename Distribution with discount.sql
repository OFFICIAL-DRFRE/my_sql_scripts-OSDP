...
select distinct cm.distributor 'KD Code',d.NAME 'KD Name',CM.POP_NAME 'Outlet Name',
...
group by cm.distributor,d.NAME ,CM.POP_NAME , SE.LDESC,cm.town+cm.locality+cm.slocality+cm.POP,cm.DOC_DATE, Cm.DOC_NO,year(cm.DOC_DATE),
 Month(cm.DOC_DATE),pj.pjp, ds.NAME,S.LDESC,c.sku,c.amount, ((C.QTY1*s.sell_factor1)+(QTY2*s.sell_factor2)+(QTY3*s.sell_factor3))
