...
Declare @docdate date='02-feb-2022'
declare @dateto date ='02-feb-2022'
...
select distinct ph.LDESC 'PJP',cd.SKU 'sku code', --pt.LDESC 'Outlet Type',
...
convert(money,ps.price_unit3) 'Price w/vat',case when w.discount_val is Null then 0 else  w.discount_val end 'discount %',sum( convert(money, round(ps.price_unit3*(1-case when w.discount_val is null then 0
...
where cd.DOC_DATE between @docdate and @dateto and  c.pjp=@pjp
group by ph.LDESC,cd.sku, pt.LDESC, s.LDESC, ps.PRICE_UNIT3,w.discount_val